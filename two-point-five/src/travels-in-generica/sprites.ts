...
function makeULpcSprite(name: string, spriteSheet: SpriteSheet): Sprite {

    function makeRow(row: number, lastCol: number, firstCol = 0): Frame[] {
        const output: Frame[] = []
        for (let index = firstCol; index < lastCol; index++) {
            output.push({ sheet: spriteSheet.id, row, col: index })
        }
        return output
    }

    function makeMixedRow(row: number, columnList: number[], spriteSheet: SpriteSheet): Frame[] {
        const output: Frame[] = []
        for (let index = 0; index < columnList.length; index++) {
            output.push({ sheet: spriteSheet.id, row, col: columnList[index] })
        }
        return output
    }
